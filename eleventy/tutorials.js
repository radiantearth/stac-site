module.exports = function (eleventyConfig) {
    eleventyConfig.addFilter(
        'tutorialLocaleFromSlug',
        getTutorialLocaleFromSlug
    );
    ...
    eleventyConfig.addFilter(
        ...
        function (collection, locale, id) {
            const tutorialId = id
                ? id
                : getTutorialIdFromPath(this.ctx.page.filePathStem);

            if (!tutorialId) return `/${locale}/tutorials`;

            const tutorial = collection.find((tutorial) => {
                return (
                    tutorial.data.tutorialId === tutorialId &&
                    tutorial.data.locale === locale
                );
            });
